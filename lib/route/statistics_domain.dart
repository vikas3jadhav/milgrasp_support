...
class StatisticsDomainCount extends StatefulWidget{

  @override
  StatisticsDomainCountState createState() => new StatisticsDomainCountState();
}

class StatisticsDomainCountState extends State<StatisticsDomainCount>{
  List<SubdomainCount> subdomainData = new List<SubdomainCount>();
  int count = 0;
  ...
  Widget getMyTicketList() {
    ...
    return Container(
        ...
        child: GridView.builder(
            itemCount: this.subdomainData == null ? 0 : this.subdomainData.length,
            gridDelegate:  SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: _crossAxisCount,childAspectRatio: _aspectRatio),
            itemBuilder: (context, item) {
              var count = subdomainData[item].count  == null
                  ? "N/A"
                  : subdomainData[item].count;
              var attended_by = subdomainData[item].subdomain  == null
                  ? "N/A"
                  : subdomainData[item].subdomain;

              var domain = attended_by.split(".");
  ...
  callAPI() async {
    ...
    final res = await ApiManager().post("Support/getSupportTicketDomainWiseCount", body);

    setState(() {
      //    this.ticketData = res;
      res["subdomain_count"].forEach((data) => {
        this.subdomainData.add(SubdomainCount.fromJSON(data)),
      });
    });
  }
...
class SubdomainCount{
  String subdomain;
  String count;

  SubdomainCount(this.subdomain, this.count);


  SubdomainCount.fromJSON(Map<String, dynamic> json) {
