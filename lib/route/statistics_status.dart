...
class StatisticsStatusCount extends StatefulWidget{

  @override
  StatisticsStatusCountState createState() => new StatisticsStatusCountState();
}

class StatisticsStatusCountState extends State<StatisticsStatusCount>{
  List<StatusCount> statusData = new List<StatusCount>();
  int count = 0;
  ...
  callAPI() async {
    ...
    final res = await ApiManager().post("Support/getSupportTicketStatusWiseCount", body);

    setState(() {
      //    this.ticketData = res;
      res["status_count"].forEach((data) => {
        this.statusData.add(StatusCount.fromJSON(data)),
      });
    });
  }
