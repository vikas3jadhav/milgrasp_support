...
class StatisticsPlatformCount extends StatefulWidget{

  @override
  StatisticsPlatformCountState createState() => new StatisticsPlatformCountState();
}

class StatisticsPlatformCountState extends State<StatisticsPlatformCount>{
  List<PlatformCount> platformData = new List<PlatformCount>();
  int count = 0;
  ...
  Widget getMyTicketList() {
    ...
    return Container(
        ...
        child: GridView.builder(
            itemCount: this.platformData == null ? 0 : this.platformData.length,
            gridDelegate:  SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: _crossAxisCount,childAspectRatio: _aspectRatio),
            itemBuilder: (context, item) {
              var count = platformData[item].count  == null
                  ? "N/A"
                  : platformData[item].count;
              var attended_by = platformData[item].platform  == null
                  ? "N/A"
  ...
  callAPI() async {
    ...
    final res = await ApiManager().post("Support/getSupportTicketPlatformWiseCount", body);

    setState(() {
      //    this.ticketData = res;
      res["platform_count"].forEach((data) => {
        this.platformData.add(PlatformCount.fromJSON(data)),
      });
    });
  }
...
class PlatformCount{
  ...
  PlatformCount(this.platform, this.count);


  PlatformCount.fromJSON(Map<String, dynamic> json) {
