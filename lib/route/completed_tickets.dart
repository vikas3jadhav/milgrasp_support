...
class CompletedTickets extends StatefulWidget {
  @override
  CompletedTicketsState createState() => new CompletedTicketsState();
}

class CompletedTicketsState extends State<CompletedTickets> {
  int count = 0;
  bool _visible = false;
  bool error=false;
  ...
  Widget getMyTicketList() {
    return  ticketData.isNotEmpty ? Column(children: <Widget>[
      new Container(
        height: 70.0,
                                    ...
                                    children: [
                                      ...
                                      GestureDetector(
                                          onTap: () {
                                            final url =
                                            Uri.encodeFull(
                                                'mailto:${email}?subject=Bug Status&body=Dear ${fromuser},');
                                            UrlLauncher.launch(url);
                                          },
                                          child: Image.asset(
                                              Img.get('email.png'),
  ...
  callAPI(String searchkey) async {
    ...
    final res = await ApiManager().post("Support/getCompletedSupportTickets", body);
