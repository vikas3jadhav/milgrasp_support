...
class StatisticsAssignToCount extends StatefulWidget{

  @override
  StatisticsAssignToCountState createState() => new StatisticsAssignToCountState();
}

class StatisticsAssignToCountState extends State<StatisticsAssignToCount>{
  List<AssignToCount> assignToData = new List<AssignToCount>();
  int count = 0;
  ...
  Widget getMyTicketList() {
    ...
    return Container(
        ...
        child: GridView.builder(
            itemCount: this.assignToData == null ? 0 : this.assignToData.length,
            gridDelegate:  SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: _crossAxisCount,childAspectRatio: _aspectRatio),
            itemBuilder: (context, item) {
              var count = assignToData[item].count == null
                  ? "N/A"
                  : assignToData[item].count;
              var attended_by = assignToData[item].assignedusername == null
                  ? "N/A"
                  : assignToData[item].assignedusername;
              return Container(
                  child:Card(
  ...
  callAPI() async {
    ...
    final res = await ApiManager().post("Support/getSupportTicketsByAssignTo", body);

    setState(() {
      //    this.ticketData = res;
      res["status_with_count"].forEach((data) => {
        this.assignToData.add(AssignToCount.fromJSON(data)),
      });
    });
  }
...
class AssignToCount{
  ...
  AssignToCount(this.assignedusername, this.count);


  AssignToCount.fromJSON(Map<String, dynamic> json) {
